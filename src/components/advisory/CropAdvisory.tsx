...
export default function CropAdvisory() {
  ...
  const priority = advisory?.priority ?? 'Monitor'
  const priorityStyles: Record<string, { color: string; bg: string; border: string }> = {
    Urgent: { color: '#f87171', bg: 'rgba(248,113,113,0.10)', border: 'rgba(248,113,113,0.25)' },
    'Act today': { color: '#fbbf24', bg: 'rgba(251,191,36,0.10)', border: 'rgba(251,191,36,0.25)' },
    Monitor: { color: '#4ade80', bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.22)' },
  }
  const priorityStyle = priorityStyles[priority] ?? priorityStyles.Monitor
      ...
      {crops.length === 0 ? (
        ...
        <>
          ...
          {loading ? (
            ...
            <>
              <div className="p-3.5 rounded-xl mb-3" style={{ background: priorityStyle.bg, border: `1px solid ${priorityStyle.border}` }}>
                <div className="flex items-center justify-between gap-3 mb-1.5">
                  <p className="text-xs font-semibold" style={{ color: priorityStyle.color }}>✦ AI field action</p>
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ color: priorityStyle.color, border: `1px solid ${priorityStyle.border}` }}>{priority}</span>
                </div>
                <p className="text-xs leading-relaxed text-white">{advisory.nextAction ?? 'Review the crop recommendations below.'}</p>
                {advisory.weatherAlert && <p className="text-xs mt-2" style={{ color: 'rgba(255,255,255,0.5)' }}>🌦️ {advisory.weatherAlert}</p>}
              </div>
