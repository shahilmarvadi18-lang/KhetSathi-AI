...
export default function PriceTrends() {
  ...
  const rawPrice = currentPrice?.price ? Number(currentPrice.price.replace(/[^0-9]/g, '')) || 2000 : 2000
  ...
  const trend = useMemo(() => generateIllustration(rawPrice), [rawPrice, selected])
  ...
  return (
    <div className="rounded-xl p-5" style={{ background: 'white', border: '1px solid rgba(0,0,0,0.08)' }}>
      ...
      {loading ? <div className="h-32 animate-pulse rounded-lg" style={{ background: 'rgba(0,0,0,0.04)' }} /> : error ? (
        <div className="h-48 flex items-center justify-center text-center text-xs px-8" style={{ color: '#8a8a7a' }}>
          Price trend will be available when the live Mandi feed reconnects.
        </div>
      ) : (
        <>
