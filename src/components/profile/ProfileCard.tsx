  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'white', border: '1px solid rgba(0,0,0,0.1)' }}>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="mb-5 p-3.5 rounded-xl" style={{ background: 'linear-gradient(135deg, rgba(22,163,74,0.08), rgba(74,222,128,0.03))', border: '1px solid rgba(22,163,74,0.16)' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold" style={{ color: '#166534' }}>Farm snapshot</span>
            <span className="text-xs" style={{ color: '#16a34a' }}>● Active</span>
          </div>
          <p className="text-sm font-semibold" style={{ color: '#1a1a14' }}>{profile?.farm_name ?? 'Set up your farm'}</p>
          <p className="text-xs mt-1" style={{ color: '#6a6a5a' }}>
            📍 {[profile?.district, profile?.state].filter(Boolean).join(', ') || 'Gandhinagar, Gujarat'}
          </p>
          <div className="flex items-center gap-2 mt-3 text-xs" style={{ color: '#4a4a3a' }}>
            <span className="px-2 py-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.7)' }}>🌾 {profile?.crops?.length ?? 0} crops</span>
            <span className="px-2 py-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.7)' }}>📐 {profile?.land_size || '—'} acres</span>
          </div>
        </div>
