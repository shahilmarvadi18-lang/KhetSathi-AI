  ...
  return (
    <main className="relative min-h-screen" style={{ background: '#f5f0e8' }}>
      ...
      <div className="relative max-w-5xl mx-auto px-5 sm:px-8 py-8">
        ...
        <h1 className="font-serif text-4xl mt-3" style={{ color: '#1a1a14' }}>What needs attention today?</h1>
        <p className="text-sm mt-2 mb-7" style={{ color: '#6a6a5a' }}>Weather-based actions for {location?.display ?? DEFAULT_LOCATION.display}.</p>
        {loading ? <div className="h-52 rounded-2xl animate-pulse" style={{ background: 'white' }} /> : <div className="space-y-4">{alerts.map(alert => <article key={alert.title} className="p-5 rounded-2xl flex items-start gap-4" style={{ background: 'white', border: `1px solid ${alert.color}24` }}><div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0" style={{ background: `${alert.color}12` }}>{alert.icon}</div><div className="flex-1"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold" style={{ color: '#1a1a14' }}>{alert.title}</h2><span className="text-xs px-2 py-1 rounded-full" style={{ color: alert.color, background: `${alert.color}10` }}>{alert.level}</span></div><p className="text-sm leading-relaxed mt-2" style={{ color: '#6a6a5a' }}>{alert.detail}</p></div></article>)}</div>}
